...
public class GeodeAggregateRel extends Aggregate implements GeodeRel {
  ...
  public Aggregate copy(RelTraitSet traitSet, RelNode input, boolean indicator,
    ...
    return new GeodeAggregateRel(getCluster(), traitSet, input, indicator, groupSet, groupSets, aggCalls);
  }
  ...
  @Override public void implement(GeodeImplementContext geodeImplementContext) {
    ...
    for (AggregateCall aggCall : aggCalls) {
      ...
      // but allows it for count('any column name'). So we are converting the count(*) into count(first input
      // ColumnName).
    ...
    geodeImplementContext.addAgregateFunctions(aggregateFunctionMap.build());
...
//  private String inputFieldName(int index) {
