...
public class GeodeTable extends AbstractQueryableTable implements TranslatableTable {
  ...
  /**
   * Executes a OQL query on the underlying table. Called by the GeodeQueryable which in turn is
   * called via the
   * generated code.
   *
  ...
  public static class GeodeQueryable<T> extends AbstractTableQueryable<T> {
    ...
    public Enumerator<T> enumerator() {
      throw new UnsupportedOperationException("Enumberator on Queryable should never be called "
          + "(tzolov)!");
    }
    ...
    public Enumerable<Object> query(
        ...
        List<String> order,
        String limit) {
      return getTable().query(getClientCache(), fields, selectFields, aggregateFunctions,
          groupByFields,
